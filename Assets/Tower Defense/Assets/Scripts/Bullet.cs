...
{
    private float _bulletPower; // 🚨 ALTERADO PARA float
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.Equals(_targetEnemy.gameObject))
        {
            ...
            {
                // ExplodeAt no LevelManager deve ser ajustado para aceitar float se necessário
                // Mas a chamada aqui usa _bulletPower (float)
                LevelManager.Instance.ExplodeAt(transform.position, _bulletSplashRadius, Mathf.RoundToInt(_bulletPower)); // 🚨 CONVERTENDO PARA INT
            }

            // Bullet yang hanya single-target
            else
            {
                _targetEnemy.ReduceEnemyHealth(_bulletPower); // 🚨 PASSANDO FLOAT
            }
            _targetEnemy = null;
        }
    }

    // 🚨 ALTERADO PARA float (bulletPower)
